...
#    }
	...
	$infoDir = $Sys->Get('INFO');
		...
		# 認証用文字列検索用パス指定
		my $authPath = "${authDir}${authNumberFilePass}";
		...
		if ($Set->Equal('BBS_NINJA', 'checked')){
			...
			if (-f $ssPath && $count >= $authNinKakikomi){
				...
				if (-f $authPath){
					# 認証文字列ファイルの更新時間取得
					my $authfiletime = localtime((stat("${authPath}"))[9]);
					# 現在の時間取得
					...
					if ($authtimelag < $authtimelimit){
						...
						$mailsage = "sage";
						$Form->Set('mail', $mailsage);
						# 初書き込み時名前変更
						# 認証時の名前設定
						my $authwritingname = " ★ 認証成功！";
						# スレ属性取得
						my $threadid = $Sys->Get('KEY');
						my $noAttr = $Sec->IsAuthority($capID, $ZP::CAP_REG_NOATTR, $Form->Get('bbs'));
						# 名前取得
						...
						if (($authwriting eq ''||$Threads->GetAttr($threadid,'force774')) && !$noAttr) {
							if($Threads->GetAttr($threadid,'change774')){
								$authwriting = $Threads->GetAttr($threadid,'change774');
							}
							else{
								$authwriting = $this->{'SET'}->Get('BBS_NONAME_NAME');
							}
						}
						else {
							# 何もスレ属性が付いていなかったら
							my $authwtiringelse = $Form->Get('FROM');
							$authwriting = $authwtiringelse
						}
		...
		# 忍法帖が有効になってない場合の認証処理を入れたい場合は以下の中身を改造してください。
				...
				if (($NoNinpochoName eq ''||$Threads->GetAttr($threadid,'force774')) && !$noAttr) {
					if($Threads->GetAttr($threadid,'change774')){
						$NoNinpochoName = $Threads->GetAttr($threadid,'change774');
					}
					else{
					...
					$NoNinpochoName = $NoNinpochoNameelse
				}
				# 認証時の名前結合
